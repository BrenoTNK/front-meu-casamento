...
const Buffet = () => {
  ...
  const buffet = route.params?.item;

  const [isFavorite, setIsFavorite] = useState(buffet.favorite);
  const [quantity, setquantity] = useState(1);

  const handleToggleFavorite = () => {
    setIsFavorite(!isFavorite);

    buffet.favorite = !buffet.favorite;
  };
  ...
  const calcularTotal = () => {
    if (buffet.sale?.value) {
      const value = (buffet.value * (100 - buffet.sale.value)) / 100;

      return value * quantity;
    }
    return buffet.value * quantity;
  };
    ...
    <Container fluid className="overflow-auto">
      <Row>
        ...
        <Col className="p-4" xs={ 12 } sm={ 12 } md={ 12 } lg={ 6 } xl={ 6 } xxl={ 6 }>
          <h3 className="fw-bold">Restaurante</h3>
          ...
          <h4 className="fw-bold d-flex justify-content-between">
            ...
            <span>
              <span className={ buffet.sale?.value ? "text-danger text-decoration-line-through" : '' }>
                R${ formatNumberBR(buffet.value) }
              </span>
              { buffet.sale?.value ? ` R$${ formatNumberBR((buffet.value * (100 - buffet.sale.value)) / 100) }` : '' }
              /{ buffet.uni }
            </span>
          </h4>
        </Col>
