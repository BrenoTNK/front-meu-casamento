...
const Gift = () => {
  ...
  const gift = route.params?.item;

  const [isFavorite, setIsFavorite] = useState(gift.favorite);
  const [quantity, setquantity] = useState(1);

  const handleToggleFavorite = () => {
    setIsFavorite(!isFavorite);

    gift.favorite = !gift.favorite;
  };
  ...
  const calcularTotal = () => {
    if (gift.sale?.value) {
      const value = (gift.value * (100 - gift.sale.value)) / 100;

      return value * quantity;
    }
    return gift.value * quantity;
  };
      ...
      <Row>
        ...
        <Col className="p-4" xs={ 12 } sm={ 12 } md={ 12 } lg={ 6 } xl={ 6 } xxl={ 6 }>
          ...
          <h4 className="fw-bold d-flex justify-content-between">
            ...
            <span>
              <span className={ gift.sale?.value ? "text-danger text-decoration-line-through" : '' }>
                R${ formatNumberBR(gift.value) }
              </span>
              { gift.sale?.value ? ` R$${ formatNumberBR((gift.value * (100 - gift.sale.value)) / 100) }` : '' }
              /{ gift.uni }
            </span>
          </h4>
        </Col>
