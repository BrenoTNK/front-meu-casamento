...
const Clouth = () => {
  ...
  const clouth = route.params?.item;

  const [isFavorite, setIsFavorite] = useState(clouth.favorite);
  const [quantity, setquantity] = useState(1);

  const handleToggleFavorite = () => {
    setIsFavorite(!isFavorite);

    clouth.favorite = !clouth.favorite;
  };
  ...
  const calcularTotal = () => {
    if (clouth.sale?.value) {
      const value = (clouth.value * (100 - clouth.sale.value)) / 100;

      return value * quantity;
    }
    return clouth.value * quantity;
  };
    ...
    <Container fluid className="overflow-auto">
      <Row>
        ...
        <Col className="p-4" xs={ 12 } sm={ 12 } md={ 12 } lg={ 6 } xl={ 6 } xxl={ 6 }>
          <h3 className="fw-bold">Estilista</h3>
          ...
          <h4 className="fw-bold d-flex justify-content-between">
            ...
            <span>
              <span className={ clouth.sale?.value ? "text-danger text-decoration-line-through" : '' }>
                R${ formatNumberBR(clouth.value) }
              </span>
              { clouth.sale?.value ? ` R$${ formatNumberBR((clouth.value * (100 - clouth.sale.value)) / 100) }` : '' }
              /{ clouth.uni }
            </span>
          </h4>
        </Col>
