...
const Cake = () => {
  ...
  const cake = route.params?.item;

  const [isFavorite, setIsFavorite] = useState(cake.favorite);
  const [quantity, setquantity] = useState(1);

  const handleToggleFavorite = () => {
    setIsFavorite(!isFavorite);

    cake.favorite = !cake.favorite;
  };
  ...
  const calcularTotal = () => {
    if (cake.sale?.value) {
      const value = (cake.value * (100 - cake.sale.value)) / 100;

      return value * quantity;
    }
    return cake.value * quantity;
  };
    ...
    <Container fluid className="overflow-auto">
      <Row>
        ...
        <Col className="p-4" xs={ 12 } sm={ 12 } md={ 12 } lg={ 6 } xl={ 6 } xxl={ 6 }>
          <h3 className="fw-bold">Confeitaria</h3>
          ...
          <h4 className="fw-bold d-flex justify-content-between">
            ...
            <span>
              <span className={ cake.sale?.value ? "text-danger text-decoration-line-through" : '' }>
                R${ formatNumberBR(cake.value) }
              </span>
              { cake.sale?.value ? ` R$${ formatNumberBR((cake.value * (100 - cake.sale.value)) / 100) }` : '' }
              /{ cake.uni }
            </span>
          </h4>
        </Col>
