...
const Party = () => {
  ...
  const party = route.params?.item;

  const [isFavorite, setIsFavorite] = useState(party.favorite);
  const [quantity, setquantity] = useState(1);

  const handleToggleFavorite = () => {
    setIsFavorite(!isFavorite);

    party.favorite = !party.favorite;
  };
  ...
  const calcularTotal = () => {
    if (party.sale?.value) {
      const value = (party.value * (100 - party.sale.value)) / 100;

      return value * quantity;
    }
    return party.value * quantity;
  };
      ...
      <Row>
        ...
        <Col className="p-4" xs={ 12 } sm={ 12 } md={ 12 } lg={ 6 } xl={ 6 } xxl={ 6 }>
          ...
          <h4 className="fw-bold d-flex justify-content-between">
            ...
            <span>
              <span className={ party.sale?.value ? "text-danger text-decoration-line-through" : '' }>
                R${ formatNumberBR(party.value) }
              </span>
              { party.sale?.value ? ` R$${ formatNumberBR((party.value * (100 - party.sale.value)) / 100) }` : '' }
              /{ party.uni }
            </span>
          </h4>
        </Col>
